...
//     const shareLink = `http://localhost:3000/sign/${report.shareToken}`;
...
// // Get report data by share token (for customer view)
...
// // Customer submits their signature + remarks
...
//     // Only update the customer-facing fields
...
exports.submitEngineerSignature = async (req, res) => {
  try {
    ...
    const { engineeringName, engineeringDate, engineeringSignature } = req.body;
    ...
    // Regenerate PDF with engineering signature
    try {
      await generatePDF(report);
    } catch (pdfError) {
